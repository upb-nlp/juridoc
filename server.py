...
async def annotate_document_with_llm_wrapper(task_id: str, document: DocumentRequest):
    """Background task wrapper to process a document using LLM and annotate it."""
    await annotate_document_with_llm(task_id, document, update_task_status)

async def summarize_document_with_llm(task_id: str, document: DocumentRequest):
    """Background task to process a document and generate a summary."""
    await summarize_document_categories(task_id, document, update_task_status)

@app.post("/annotate-document", response_model=TaskResponse)
async def annotate_document(document: DocumentRequest, background_tasks: BackgroundTasks):
    """
    ...
    background_tasks.add_task(annotate_document_with_llm_wrapper, task_id, document)
...
async def summarize_document(document: DocumentRequest, background_tasks: BackgroundTasks):
    """
    ...
    background_tasks.add_task(summarize_document_with_llm, task_id, document)
