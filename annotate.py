...
async def annotate_document_with_llm(task_id: str, document: DocumentRequest, update_task_status_callback):
    ...
    try:
        ...
        update_task_status_callback(task_id, TaskStatus.ANNOTATING, "Annotating document using LLM")
        ...
        # Get the document type for prompts
        doc_type = SUPPORTED_DOCUMENT_TYPES[document.documentTypeName]
        
        async def process_annotation_type(annotation_type: str):
            try:
                ...
                print(f"Processing {annotation_type} with model {model_name}")
                ...
                return annotation_type, result

            except Exception as e:
                print(f"Error processing {annotation_type}: {str(e)}")
                return annotation_type, ""
        
        update_task_status_callback(task_id, TaskStatus.ANNOTATING, f"Processing annotation types: {', '.join(annotation_types)}")
        
        # Create tasks for all annotation types, we run them in parallel to leverage prefix caching
        ...
        for result in annotation_results_list:
            if isinstance(result, Exception):
                ...
                annotation_type, response_content = result
                annotation_results[annotation_type] = response_content
        ...
        for annotation_type, result_text in annotation_results.items():
            ...
            for para_content in matches:
                ...
                # Find the paragraph with the highest match score algorithmically
