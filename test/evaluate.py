...
class DocumentEvaluator:
    ...
    ENTITY_TYPES = ['isReclamant'] 
    
    def __init__(self, server_url: str, timeout: int = 300, max_workers: int = 3):
    ...
    def prepare_document_for_processing(self, validation_doc: Dict[str, Any], extraction_types: List[str] = None) -> Dict[str, Any]:
        ...
        # Add extraction types if specified
        if extraction_types:
        ...
        for page in document['pages']:
            for paragraph in page.get('paragraphs', []):
                for word in paragraph.get('words', []):
                    ...
                    # Get annotation status for each entity type
        ...
        for page in document['pages']:
            for paragraph in page.get('paragraphs', []):
                for word in paragraph.get('words', []):
                    ...
                    # Check if any word is annotated for any of the specified entity types
    ...
    def process_single_document(self, validation_doc: Dict[str, Any], doc_index: int, total_docs: int, extraction_types: List[str] = None) -> Tuple[int, Optional[Dict[str, Dict[str, Any]]], str, Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            # Prepare document for processing (sets all annotations to False)
            ...
            # Evaluate this document using word-level comparison
    ...
    def calculate_word_level_metrics(self, ground_truth_annotations: List[bool], predicted_annotations: List[bool]) -> Dict[str, float]:
        ...
        # Percentage of originally annotated words that were correctly identified
        recall_percentage = (tp / gt_positive_words * 100) if gt_positive_words > 0 else 0.0
        
        # Extra annotations (false positives)
    ...
    def evaluate_document_pair(self, ground_truth_doc: Dict[str, Any], predicted_doc: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        # Extract word-level annotations from both documents
        ...
        # Extract the actual annotated words
        ...
        for entity_type in self.ENTITY_TYPES:
            ...
            # Add the annotated words to the results
    ...
    def aggregate_metrics(self, all_document_results: List[Dict[str, Dict[str, Any]]]) -> Dict[str, Dict[str, Any]]:
        ...
        for entity_type in self.ENTITY_TYPES:
            # Collect all metrics for this entity type
            all_metrics = {
            ...
            for doc_result in all_document_results:
                if entity_type in doc_result:
                    ...
                    # Only collect numerical metrics for aggregation (skip word strings)
    ...
    def evaluate_all_documents(self, validation_dir: str, extraction_types: List[str] = None) -> EvaluationResult:
        ...
        # Filter out documents that have no annotations for the specified entity types (only if not pre-filtered)
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_doc):
                ...
                if doc_results is not None:
                    ...
                    # Add failed document to detailed results for completeness
...
def main():
    ...
    evaluator = DocumentEvaluator(server_url=args.server_url, timeout=args.timeout, max_workers=args.max_workers)
    
    try:
        if args.limit:
            validation_data = evaluator.load_validation_data(args.validation_dir)
            
            # Use specified extraction types or default to all for filtering
            extraction_types_for_filtering = args.extraction_types if args.extraction_types else evaluator.ENTITY_TYPES
            
            # Filter documents that have annotations for the specified entity types
