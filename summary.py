...
async def generate_category_summary(category_text: str, annotation_type: str, document_type: str = "subpoena") -> str:
    """Generate a summary for a specific category using the extracted category text"""
    try:
        if not category_text.strip():
            return ""
        
        # Get the summary prompts for the document type
        ...
        user_content = build_user_prompt_for_task_type(category_text, annotation_type, "summary", document_type)
...
async def summarize_document_categories(task_id: str, document: DocumentRequest, update_task_status_callback):
    ...
    try:
        summary_annotation_types = ['isTemei', 'isProba', 'isSelected', 'isCerere']
        ...
        async def process_category_summary(annotation_type: str):
            ...
            try:
                ...
                # Generate summary from the extracted category text
                summary_text = await generate_category_summary(category_text, annotation_type, doc_type)
